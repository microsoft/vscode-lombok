...
public class GetterSetterHandler {
	...
	private static void generateGetterSetterMethods(IType type, ListRewrite listRewriter, AccessorField accessor, ASTNode insertion) throws OperationCanceledException, CoreException {
	...
	private static void insertMethod(IField field, ListRewrite rewrite, AccessorKind kind, ASTNode insertion) throws CoreException {
				...
				switch (kind) {
					case BOTH:
						if (generateGetter || generateSetter) {
							implemented.add(new AccessorField(field.getElementName(), isStatic, generateGetter, generateSetter, Signature.getSignatureSimpleName(field.getTypeSignature())));
						}
						break;
					case GETTER:
						if (generateGetter) {
							implemented.add(new AccessorField(field.getElementName(), isStatic, generateGetter, false, Signature.getSignatureSimpleName(field.getTypeSignature())));
						}
						break;
					case SETTER:
						if (generateSetter) {
							implemented.add(new AccessorField(field.getElementName(), isStatic, false, generateSetter, Signature.getSignatureSimpleName(field.getTypeSignature())));
						}
						break;
					default:
						break;
        ...
        try {
		    ...
		    for (IField field : fields) {
			    ...
			    if (!Flags.isEnum(flags)) {
				    ...
				    switch (kind) {
					    case BOTH:
                            accessors.add(new AccessorField(field.getElementName(), isStatic, true, true, Signature.getSignatureSimpleName(field.getTypeSignature())));
						    break;
					    case GETTER:
                            accessors.add(new AccessorField(field.getElementName(), isStatic, true, false, Signature.getSignatureSimpleName(field.getTypeSignature())));
						    break;
					    case SETTER:
                            accessors.add(new AccessorField(field.getElementName(), isStatic, false, true, Signature.getSignatureSimpleName(field.getTypeSignature())));
						    break;
					    default:
						    break;
				    }
